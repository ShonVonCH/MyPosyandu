...
class DatabaseHelper(context: Context) :
    ...
    companion object {
        ...
        private const val SQL_CREATE_PEMERIKSAAN = """
            CREATE TABLE IF NOT EXISTS $TABLE_PEMERIKSAAN (
                ...
                FOREIGN KEY($COL_PMRK_ANAK_ID)
                    REFERENCES $TABLE_ANAK($COL_ANAK_ID),
                FOREIGN KEY($COL_PMRK_KADER_ID)
                    REFERENCES $TABLE_USERS($COL_USERS_ID)
            )
        """

        private const val SQL_CREATE_IMUNISASI = """
            CREATE TABLE IF NOT EXISTS $TABLE_IMUNISASI (
                $COL_IMN_ID INTEGER PRIMARY KEY AUTOINCREMENT,
                $COL_IMN_NAMA_ANAK TEXT NOT NULL,
                $COL_IMN_NIK_ANAK TEXT,
                $COL_IMN_NAMA_ORTU TEXT,
                $COL_IMN_NAMA_VAKSIN TEXT NOT NULL,
                $COL_IMN_TGL_VAKSIN TEXT,
                $COL_IMN_STATUS INTEGER DEFAULT 0
            )
        """
        ...
        private const val SQL_CREATE_VAKSIN_RIWAYAT = """
            CREATE TABLE IF NOT EXISTS $TABLE_VAKSIN_RIWAYAT (
                ...
                $COL_VR_LOKASI TEXT,
                FOREIGN KEY($COL_VR_ANAK_ID) REFERENCES $TABLE_ANAK($COL_ANAK_ID),
                FOREIGN KEY($COL_VR_VAKSIN_REF_ID) REFERENCES $TABLE_VAKSIN_REF($COL_VAKSIN_REF_ID),
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL(SQL_CREATE_IMUNISASI)
